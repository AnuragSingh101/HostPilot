...
const Input = ({ name, placeholder, type = 'text', value, onChange }) => (
  <input
    ...
    value={value}
    onChange={onChange}
    className="bg-gray-800 text-gray-200 border border-gray-700 px-4 py-3 rounded-lg w-full shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300 placeholder:text-gray-400"
  />
);
...
const Signup = () => {
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-2 bg-gray-950 text-gray-200">
      ...
      {/* LEFT */}
      <div className="bg-gradient-to-br from-gray-900 to-gray-950 flex flex-col justify-between p-10 relative border-r border-gray-800">
        <div className="text-blue-400 text-2xl font-extrabold absolute top-6 left-6">HostPilot</div>

        <div className="flex-1 flex flex-col justify-center mt-10">
          <h1 className="text-4xl font-bold mb-4 leading-snug text-white">Build Your Project with HostPilot</h1>
          <p className="text-base text-gray-400 mb-8">
            Powerful, secure, and flexible infrastructure to get your apps live — fast.
          </p>
          <ul className="list-disc pl-5 text-sm space-y-2 mb-10 text-gray-400">
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-md max-w-md border border-gray-700">
            <p className="italic mb-2 text-gray-300">“Onboarded in minutes!”</p>
            <p className="text-sm text-gray-400">Great UI, flexible hosting, and seamless experience all in one place.</p>
            <p className="mt-4 text-sm font-semibold text-right text-blue-400">— Anurag Singh ⭐⭐⭐⭐⭐</p>
          </div>
        </div>
      ...
      {/* RIGHT */}
      <div className="p-10 flex flex-col justify-center relative bg-gray-900">
        <div className="text-blue-400 text-2xl font-bold absolute top-6 left-6">HostPilot</div>
        <h2 className="text-3xl font-bold mb-6 text-white mt-16">Sign Up for Free</h2>
          ...
          {step === 1 && (
            <>
              ...
              <button
                type="button"
                onClick={() => setStep(2)}
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-md w-full transition-all duration-300"
              >
                Next →
              </button>
            </>
            ...
            <>
              <div className="space-y-4">
                ...
                <Listbox value={form.accountType} onChange={(value) => setForm({ ...form, accountType: value })}>
                  <div className="relative">
                    <Listbox.Button className="border border-gray-700 px-4 py-3 rounded-md w-full text-left bg-gray-800 text-gray-200 font-medium">
                      {accountTypes.find((t) => t.value === form.accountType)?.label || 'Select Type'}
                    </Listbox.Button>
                    <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
                      <Listbox.Options className="absolute mt-1 w-full bg-gray-800 border border-gray-700 rounded-md z-10">
                        {accountTypes.map((type) => (
                          <Listbox.Option key={type.value} value={type.value} className="cursor-pointer px-4 py-2 hover:bg-gray-700">
                            {type.label}
                          </Listbox.Option>
                        ))}
                      </Listbox.Options>
              ...
              <div className="flex justify-between gap-4 mt-6">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="border border-gray-600 text-gray-300 py-3 px-6 rounded-md w-full hover:bg-gray-800 transition-all duration-300"
                >
                  ← Back
                </button>
                <button
        ...
        <div className="mt-6 bg-gray-800 text-blue-400 p-3 rounded text-center text-sm font-medium border border-gray-700">
