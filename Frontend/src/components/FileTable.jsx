...
const FileTable = ({ files = [], onDelete, onRename, onNavigate, onOpen }) => (
      ...
      <tbody>
        {files.length > 0 ? files.map((file, idx) => (
          <tr key={idx} className="border-t border-gray-800 hover:bg-gray-800/70 transition">
            ...
            >
              {file.type === 'folder' ? <FaFolder className="text-yellow-400" /> : <FaFile className="text-gray-400" />}
              <span>{file.name}</span>
            </td>
            ...
            <td className="px-4 py-2 text-right space-x-4">
              {file.type === 'file' && (
                <button onClick={(e) => { e.stopPropagation(); onOpen(file); }} className="text-green-400 hover:underline">
                  Open
                </button>
              )}
              <button onClick={(e) => { e.stopPropagation(); onRename(file); }} className="text-yellow-400 hover:underline">
                Rename
              </button>
              <button onClick={(e) => { e.stopPropagation(); onDelete(file); }} className="text-red-400 hover:underline">
                Delete
              </button>
            </td>
          </tr>
          ...
          <tr>
            <td colSpan="3" className="px-4 py-2 text-center text-gray-500">No files found</td>
          </tr>
        )}
      </tbody>
    </table>
