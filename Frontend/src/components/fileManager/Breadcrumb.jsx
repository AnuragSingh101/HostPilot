...
const Breadcrumb = ({ path = '', onNavigate }) => {
  ...
  return (
    <nav className="flex items-center text-sm text-gray-300 mb-4 bg-gray-800 px-3 py-2 rounded-lg shadow border border-gray-800">
      ...
      <span onClick={() => onNavigate('/')} className="cursor-pointer hover:text-blue-400 transition">Root</span>
      {parts.map((part, idx) => {
        const subPath = '/' + parts.slice(0, idx + 1).join('/');
        return (
          <span key={idx} className="flex items-center">
            <span className="mx-2 text-gray-600 font-bold">/</span>
            <span onClick={() => onNavigate(subPath)} className="cursor-pointer hover:text-blue-400 transition">
              {part}
            </span>
          </span>
        );
